...
void dfsUtil(int vertex, unordered_map<int, vector<int>>& graph, unordered_map<int, char>& bijection, set<int>& visited, string& result) {
    visited.insert(vertex);
    result += bijection[vertex];
    for (const int& neighbor : graph[vertex]) {
        if (visited.find(neighbor) == visited.end()) {
            dfsUtil(neighbor, graph, bijection, visited, result);
        }
    }
}
...
string dfs(unordered_map<int, vector<int>>& graph, int start, unordered_map<int, char>& bijection) {
    set<int> visited;
    string result;
    dfsUtil(start, graph, bijection, visited, result);
    return result;
}
...
int main() {
    vector<pair<int, int>> edges = {{1, 2}, {2, 3}, {3, 4}, {4, 10}, {4, 5}, {5, 6}, {6, 7}, {6, 8}, {6, 2}, {5, 8}, {8, 9}, {3, 11}, {7, 8}};
