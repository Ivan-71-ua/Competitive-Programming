...
int get_sum(int a, int b, std::vector<int> &
pref_sum) {
	return pref_sum[b] - pref_sum[a - 1];
} 

void solve(int L, int R, std::vector<int> &nums, std::vector<int> &pref_sum, int &ans) {
	if(R <= L) {
		ans++;
		return;
	}
...
int32_t main() {
	...
	while(n--) {
		...
		std::vector<int> pref_sum(size + 1);
		for (int i = 1; i < size + 1; i++)
		{
			pref_sum[i] = pref_sum[i - 1] + nums[i - 1];
		}
		int res = 0;
		//std::unordered_map<int, int> sum_on_sub;
